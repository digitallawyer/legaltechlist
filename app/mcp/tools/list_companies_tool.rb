    ...
    class ListCompaniesTool < BaseTool
      ...
      input_schema(
        properties: {
          ...
          url_reason: { type: "string", description: "Filter by url_health reason_code. HTTP client errors use one code per status (http_404, http_410, http_400, ...); other codes: dns_failure, timeout, bot_blocked, tls_untrusted, ssl_error, server_error, redirect_error, connection_reset, connection_error (and, under specific conditions, connection_refused, host_unreachable, invalid_url). See the live set in get_stats.companies.url_health.by_reason. Combine with url_health_status:\"unknown\" to separate \"blocks crawlers\" from \"actually dead\"." },
